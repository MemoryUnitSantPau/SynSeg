...
%     number_channels = str2num(answer{:});
            ...
            if isempty(number_channels) || number_channels < 2
                message = sprintf('Sorry, but number of channels must be a float greater than one... Lets try again! :)');
...
% srcPath = uigetdir('/media/Elements/ANALISI_AT/imatges_prova/coloc/segmented_2016/crops','Select the sequence path'); %DIRECTORI  ON ESTAN LES IMATGES
    ...
    [x,y] = size(srcFiles);
            ...
            if x < 2 || x < number_channels
                message = sprintf('Ouch! You wanted to compare %d channels and there�s not enough images in this directory...',number_channels);
                uiwait(warndlg(message));
                return;
            end
